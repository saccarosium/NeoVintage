...
class InputStateTracker(sublime_plugin.EventListener):
    ...
    def on_query_context(self, view, key, operator, operand, match_all):
        if key == "vi_action" and g_input_state.action_command:
            ...
            v = (
                len(g_input_state.motion_repeat_digits) > 0
                or len(g_input_state.prefix_repeat_digits) > 0
                or g_input_state.action_command is not None
                or g_input_state.register is not None
            )
            if operator == sublime.OP_EQUAL:
                return v == operand
            if operator == sublime.OP_NOT_EQUAL:
            ...
            v = (
                g_input_state.action_command is not None
            ) or view.has_non_empty_selection_region()
            if operator == sublime.OP_EQUAL:
                return v == operand
            if operator == sublime.OP_NOT_EQUAL:
                return v != operand
...
class SetMotion(sublime_plugin.TextCommand):
    ...
    def run(
        ...
        if (
            not g_input_state.motion_mode_overridden
...
def set_single_line_selection_direction(view, forward):
    transform_selection_regions(
        view, lambda r: orient_single_line_region(view, forward, r)
    )
...
def set_single_character_selection_direction(view, forward):
    transform_selection_regions(
        view, lambda r: orient_single_character_region(view, forward, r)
    )
    ...
    def run(
        ...
        for i in range(prefix_repeat):
            ...
            if motion_command:
                ...
                for j in range(motion_repeat):
                    if direction != 0 and motion_mode == MOTION_MODE_LINE:
                        ...
                        set_single_character_selection_direction(
                    ...
                    if motion_mode == MOTION_MODE_LINE:
                        ...
                        transform_selection_regions(
                            self.view,
                            lambda r: (
                                sublime.Region(r.b, r.b + 1, r.xpos) if r.empty() else r
                            ),
                        )
            ...
            if (
                ...
                and action_command
                and motion_clip_to_line
            ):
                transform_selection_regions(
                    self.view, lambda r: self.view.split_by_newlines(r)[0]
                )
...
class EnterVisualMode(sublime_plugin.TextCommand):
    def run(self, edit):
        ...
        transform_selection_regions(
            self.view, lambda r: sublime.Region(r.b, r.b + 1) if r.empty() else r
        )
...
class ViPasteRight(ViPrefixableCommand):
    ...
    def run(self, edit, register='"', repeat=1):
        ...
        self.view.run_command(
            "paste_from_register",
            {"forward": not visual_mode, "repeat": repeat, "register": register},
        )
...
class ViPasteLeft(ViPrefixableCommand):
    def run(self, edit, register='"', repeat=1):
        self.view.run_command(
            "paste_from_register",
            {"forward": False, "repeat": repeat, "register": register},
        )
...
class ScrollCursorLineToTop(sublime_plugin.TextCommand):
    def run(self, edit):
        self.view.set_viewport_position(
            (self.view.viewport_position()[0], self.view.layout_extent()[1])
        )
...
class ViScrollLines(ViPrefixableCommand):
    def run(self, edit, forward=True, repeat=None):
        ...
        def transform(pt):
            ...
            y_deltas.append(
                self.view.text_to_layout(new_pt)[1] - self.view.text_to_layout(pt)[1]
            )
            return new_pt
        ...
        self.view.run_command(
            "vi_move_to_first_non_white_space_character", {"extend": visual_mode}
        )
...
class ShowAsciiInfo(sublime_plugin.TextCommand):
    def run(self, edit):
        ...
        sublime.status_message(
            "<%s> %d, Hex %s, Octal %s" % (c, ord(c), hex(ord(c))[2:], oct(ord(c)))
        )
...
class ViSelectLastSelection(sublime_plugin.TextCommand):
    def run(self, edit):
        if self.view.has_non_empty_selection_region():
            new_sel = self.view.get_regions("vintage_last_visual_selection_exited")
            self.view.add_regions(
                "vintage_last_visual_selection_exited",
...
class VisualModeSelectionTracker(sublime_plugin.EventListener):
    def on_selection_modified(self, view):
        if view.has_non_empty_selection_region():
            view.add_regions(
                "vintage_last_visual_selection", [s for s in view.sel()], ""
            )
    ...
    def run(self, direction):
        ...
        if direction == "up":
            matches = (
                i
                for i in idxs
                if cells[i][3] == y1 and cells[i][0] < x2 and cells[i][2] > x1
            )
        elif direction == "down":
            matches = (
                i
                for i in idxs
                if cells[i][1] == y2 and cells[i][0] < x2 and cells[i][2] > x1
            )
        elif direction == "right":
            matches = (
                i
                for i in idxs
                if cells[i][0] == x2 and cells[i][1] < y2 and cells[i][3] > y1
            )
        elif direction == "left":
            matches = (
                i
                for i in idxs
                if cells[i][2] == x1 and cells[i][1] < y2 and cells[i][3] > y1
            )
