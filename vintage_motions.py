...
from NeoVintage.vintage import transform_selection
from NeoVintage.vintage import transform_selection_regions
    ...
    def run(self, edit, repeat=1):
        for i in range(repeat - 1):
            self.view.run_command(
                "move", {"by": "lines", "extend": True, "forward": True}
            )
...
class ViMoveByCharactersInLine(sublime_plugin.TextCommand):
    def run(self, edit, forward=True, extend=False, visual=False):
        ...
        transform_selection(
            self.view, lambda pt: pt + delta, extend=extend, clip_to_line=(not visual)
        )
...
class ViMoveByCharacters(sublime_plugin.TextCommand):
    ...
    def run(self, edit, forward=True, extend=False, visual=False):
        delta = 1 if forward else -1
        transform_selection(
            self.view, lambda pt: self.advance(delta, visual, pt), extend=extend
        )
...
class ViMoveToHardEol(sublime_plugin.TextCommand):
    def run(self, edit, repeat=1, extend=False):
        ...
        if repeat > 1:
            for i in range(repeat - 1):
                self.view.run_command(
                    "move", {"by": "lines", "extend": extend, "forward": True}
                )

        transform_selection(
            self.view,
            lambda pt: self.view.line(pt).b,
            extend=extend,
            clip_to_line=False,
        )
...
class ViMoveToFirstNonWhiteSpaceCharacter(sublime_plugin.TextCommand):
    ...
    def run(self, edit, repeat=1, extend=False, register='"'):
        ...
        transform_selection(
            self.view, lambda pt: self.first_character(pt), extend=extend
        )
...
class ViMoveToCharacter(sublime_plugin.TextCommand):
    ...
    def run(
        self, edit, character, extend=False, forward=True, before=False, record=True
    ):
        if record:
...
class ViMoveToBrackets(sublime_plugin.TextCommand):
    ...
    def run(self, edit, repeat=1):
        ...
        if repeat == 1:
            ...
            self.view.run_command(
                "move_to", {"to": "brackets", "extend": True, "force_outer": True}
            )
        else:
...
class ViExpandToWords(sublime_plugin.TextCommand):
    def run(self, edit, outer=False, repeat=1):
        ...
        transform_selection_regions(
            self.view, lambda r: sublime.Region(r.b + 1, r.b + 1)
        )
        self.view.run_command(
            "move",
            {
...
class ViExpandToQuotes(sublime_plugin.TextCommand):
    ...
    def expand_to_quote(self, character, r):
        ...
        if (
            ...
            and first_quote == caret_pos_in_line
        ) or (first_quote > caret_pos_in_line):
...
class ViExpandToBrackets(sublime_plugin.TextCommand):
    def run(self, edit, character, outer=False):
        self.view.run_command(
            "expand_selection", {"to": "brackets", "brackets": character}
        )
        if outer:
            self.view.run_command(
                "expand_selection", {"to": "brackets", "brackets": character}
...
class ScrollCurrentLineToScreenCenter(sublime_plugin.TextCommand):
    def run(self, edit, repeat, extend=True):
        line_nr = (
            self.view.rowcol(self.view.sel()[0].a)[0]
            if int(repeat) == 1
            else int(repeat) - 1
        )
